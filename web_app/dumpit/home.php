<?php
  ...
  require_once 'includes/config.php';
   if(loggedin()){ 
    ...
    <center><h1 style="color:#3F51B5">All available requests</h1>
    <?php

    //include 'includes/config.php';
    $provider_id = getuserid();
    $query="SELECT `id`,`user_id`,`start_date`,`end_date`,`pickup_time`,`frequency`, `status` from `request_pickup` WHERE `sp_id` = '$provider_id' AND `is_confirm`='0' AND `status`='0'";
    ...
    <td align="center">
    <form id="view_admin2" class="tmp_form" method="post" >
      <input type="hidden" name="fr" value="<?php echo $f0; ?>">
      <img src="./images/ajax-loader.gif" name="wait" style="display:none" class="img-responsive">
      <button type="submit" class="btn btn-warning" name="reject" id="btnreject">Reject</button>
      <!-- <input type="submit" name="spam" value="Spam" id = "btnspam"> -->
    </form>
    </td>
    </tr>
    ...
    $query_update1 = "UPDATE report_list SET is_confirm='1' WHERE id ='$fs_submitted'";
    ...
    //     }
    /// in case of reject setting 2 to status
    if (isset($_POST['reject'])) {
    $f0_submitted = (int) $_POST['fr'];
    $query_update = "UPDATE report_list SET status='2' WHERE id ='$f0_submitted'";
    $result_update=mysqli_query($con,$query_update);
    header('Location: home.php');}
